...
* App config is `<app>/dboss.yaml`: `procfile` + `hosts` are the minimum. The console edits these same files on disk.
...
* Fez trims every template line and turns `&lt;`/`&gt;` into real tags. A multi-line code sample therefore lives in a script constant and renders as `<pre>{state.examples.<name>}</pre>` (see `EXAMPLES` in `db-help.fez`), and an inline `<placeholder>` in template text is written `&#60;placeholder&#62;`.
