...
* Host config lives at `<dir>/appboss.yaml`: `management.host` + `management.auth` are the minimum for a real host; every other host key has a sane default (`apps: ./apps`, `proxy.listen: ":80"`, `ports.range`, the `.appboss` runtime paths, the AuthCog realm, session lifetime, metrics, upstream timeouts, wake pages, log cadence). With no config file at all `appboss start` runs the default host. Validate with `appboss check -c <dir>/appboss.yaml` before starting.
