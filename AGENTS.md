...
* `Web.HealthEndpoint` (default `/.well-known/dboss/health`) is answered by the `publicHealth` filter in `./internal/proxy/filter.go` before auth: 200 while running and not draining, 503 otherwise, never wakes the app. `dboss doctor` uses `super.ListenersInRange`.
