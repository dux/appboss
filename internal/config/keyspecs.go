...
var keySpecs = map[string]KeySpec{
	...
	"pg_db":     {Block: "app", Name: "PostgreSQL databases", Description: "databases this app owns, keyed by the environment variable each one is exported as; a value is a database name on the host's server or a full postgres:// URL, a missing database is created either way, and every process gets a connection URL", Example: "{db_main: myapp_production, db_report: $REPORTS_URL}"},
