...
var commands = []command{
	{name: "start", aliases: []string{"s"}, args: "[-c path] [--login]", group: "Host session", summary: "run the host session in the foreground; Ctrl-C stops every app",
		details: []string{"Loads the config, clears every listener in ports, starts the apps that were running before, then serves the proxy, the management console and the control socket.", "With no config file in the working directory it runs the default host: listen :80, apps in ./apps, state under ./.dboss and the console off.", "On a terminal every process's output is echoed with an app/proc prefix. Under systemd only dboss's own log reaches journald; app output stays in dir/log.", "On a terminal it first prints one row per process under that same key: the URL to open for a web process, `worker` for the rest, and what state the app is in, so an app that has not started yet is still visible. The last row is a console link that lives for an hour and can be clicked more than once; a service start prints no banner and mints no link.", "On a terminal a proxy.listen port that needs root or CAP_NET_BIND_SERVICE falls back to the first free port of ports instead of exiting; under systemd the same bind failure is fatal."},
		options: []option{configOption, {"--login", "print a one-time loopback sign-in link for the console (local development)"}}},
	...
	{name: "trust", args: "", group: "Config", summary: "trust the local certificate authority behind a dev session's HTTPS",
		details: []string{"A dev session (`dboss start` in an app folder) also serves HTTPS on :443, or proxy.tls.listen, with certificates from one local authority per user, kept in the user config directory and shared by every project.", "trust adds that root to the login keychain on macOS (it asks for your password) or to the system store through sudo on Debian and Fedora. Anywhere else it prints the file to import by hand. It does nothing when the root is already trusted."}},
